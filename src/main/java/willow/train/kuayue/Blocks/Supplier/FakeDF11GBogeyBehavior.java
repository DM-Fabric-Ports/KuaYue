...
public class FakeDF11GBogeyBehavior implements MovementBehaviour {
    ...
    private IFakeDF11GBogey getTE(MovementContext context) {
        Map<BlockPos, BlockEntity> tes = context.contraption.presentTileEntities;
        if (!(tes.get(context.localPos) instanceof IFakeDF11GBogey speedForcibleTE))
            return null;
    ...
    public void tick(MovementContext context) {
        if (context.contraption.entity instanceof CarriageContraptionEntity cce) {
            ...
            Map<BlockPos, BlockEntity> tes = context.contraption.presentTileEntities;
            if (!(tes.get(context.localPos) instanceof IFakeDF11GBogey speedForcible))
                return;
    ...
    public void stopMoving(MovementContext context) {
        if (!context.world.isClientSide || !isActive(context))
            return;
        IFakeDF11GBogey speedForcible = getTE(context);
